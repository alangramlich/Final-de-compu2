...
#include <QPainterPath> // Para función texto
#include <algorithm>

using namespace std;

Texto::Texto(string pFrase, int pX, int pY, float pR, float pG, float pB){
    frase = pFrase;
    posX = pX;
    posY = pY;
...
void Texto::dibujarTexto(string pTexto, float pX, float pY, unsigned char pTam)
{
    float escala = 0.05f;
    ...
    vector<float> coordenadasX, coordenadasY; // Vectores para centrar texto en coordenada (0, 0)
